...
fn get_pokemon_name_200() {
    ...
    assert_eq!(
        ...
        Some(
            r#"{"id":"1","type":"pokemon_names","attributes":{"name":"BULBASAUR"},"links":{"self":"http://localhost:8080/v1/pokemon/names/1"}}"#
                .to_string()
        )
    );
...
fn post_pokemon_name_202() {
    ...
    assert_eq!(
        response.body_string(),
        Some(
            r#"{"id":"1","type":"pokemon_names","attributes":{"name":"DINOSAUR"},"links":{"self":"http://localhost:8080/v1/pokemon/names/1"}}"#
                .to_string()
        )
    );
