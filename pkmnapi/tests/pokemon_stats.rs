...
fn get_pokemon_stats_404() {
    ...
    assert_eq!(
        response.body_string(),
        Some(
            r#"{"data":{"id":"error_pokemon_stats","type":"errors","attributes":{"message":"Invalid Pokédex ID: 200"}}}"#
                .to_owned()
        )
    );
...
fn post_pokemon_stats_202() {
    ...
    let request = client
        ...
        .body(r#"{"data":{"type":"pokemon_stats","attributes":{"base_hp":42,"base_attack":42,"base_defence":42,"base_speed":42,"base_special":42,"types":[{"id":"0"},{"id":"0"}],"catch_rate":42,"base_exp_yield":42}}}"#)
